  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-press-start text-[#702C95] text-[9px] leading-relaxed line-clamp-2">{game.name}</p>
        <p className="font-courier-prime text-[#702C95]/70 text-xs mt-0.5 line-clamp-1">{game.shortDesc}</p>
      </div>
      <span className={`shrink-0 text-xs font-dokdo px-2 py-0.5 rounded-full font-bold ${statusColors[game.status] ?? "bg-white text-[#702C95]"}`}>
        {game.status}
      </span>
    </a>
  );
...
function MobileProjectCard({ project }: { project: Project }) {
  const href = `/${project.category === "animatronics" ? "animatronics" : "projects"}/${project.slug}`;
  return (
    <a
      href={href}
      className="flex items-center gap-3 p-3 rounded-xl border-2 border-[#702C95] bg-[#FAF0DD]/90 active:scale-[0.98] transition-transform"
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-press-start text-[#702C95] text-[9px] leading-relaxed line-clamp-2">{project.name}</p>
        <p className="font-courier-prime text-[#702C95]/70 text-xs mt-0.5 line-clamp-1">{project.shortDesc}</p>
      </div>
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none" className="shrink-0">
        <path d="M3 8h10M9 4l4 4-4 4" stroke="#702C95" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    </a>
  );
}

function MobileIllustrationGrid({ items }: { items: Artwork[] }) {
  const [selected, setSelected] = useState<Artwork | null>(null);
  const [visible, setVisible] = useState(false);
    ...
    <>
      <div className="grid grid-cols-2 gap-2">
        {items.map((artwork) => (
          <div
            key={artwork.id}
            onClick={() => openLightbox(artwork)}
            className="aspect-square rounded-xl overflow-hidden border-2 border-[#702C95] cursor-pointer active:scale-[0.97] transition-transform"
          >
      ...
      {selected && (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className={`relative max-w-sm w-full max-h-[90vh] flex flex-col rounded-2xl overflow-hidden border-4 border-[#FDD23B] shadow-2xl transition-all duration-300 ${
              visible ? "scale-100 opacity-100" : "scale-50 opacity-0"
            }`}
          >
            <img
              src={selected.imageSrc}
              alt={selected.title}
              className="w-full object-contain max-h-[65vh] bg-[#FAF0DD]"
            />
            <div className="bg-[#FAF0DD] p-4 flex items-start justify-between gap-4">
              <div>
                <p className="font-vt323 text-2xl text-black leading-tight">{selected.title}</p>
                ...
                {selected.link && (
                  <a href={selected.link} target="_blank" rel="noopener noreferrer" className="inline-block mt-2 text-[#702C95] text-sm font-courier-prime">
                    ↗ view post
                  </a>
                )}
              </div>
              <button
                onClick={closeLightbox}
                className="shrink-0 w-8 h-8 rounded-full bg-[#702C95] text-white flex items-center justify-center text-lg leading-none"
              >
                ×
              </button>
            </div>
  ...
  return (
    <div>
      ...
      >
        ...
        <span
          className="text-2xl font-bold leading-none ml-4 shrink-0 transition-transform duration-300"
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden">
      ...
      <div className="sm:hidden">
        ...
        {/* mobile header */}
        <header className="px-6 pt-8 pb-6 bg-[#FAF0DD]/90 border-b-2 border-[#702C95]">
          <div className="text-center mb-5">
            <h1 className="font-press-start text-[#702C95] text-xl text-outline mb-3 leading-relaxed">
              Lamar Jambi
            </h1>
        ...
        <main className="px-5 py-7 space-y-3">

          <MobileAccordionSection
            label="Games + Demos"
            isOpen={mobileOpen === "games"}
            onToggle={() => toggleMobileSection("games")}
            accent="#702C95"
          >
            {games.map((game) => (
              <MobileGameCard key={game.slug} game={game} />
            ))}
          </MobileAccordionSection>

          <MobileAccordionSection
            label="Apps / Software"
            isOpen={mobileOpen === "apps"}
            onToggle={() => toggleMobileSection("apps")}
            accent="#547DFD"
          >
          ...
          <MobileAccordionSection
            label="Illustrations"
            isOpen={mobileOpen === "illustrations"}
            onToggle={() => toggleMobileSection("illustrations")}
            accent="#EC6BA7"
          >
            <MobileIllustrationGrid items={artworks} />
          </MobileAccordionSection>

          <MobileAccordionSection
            label="Animatronics"
            isOpen={mobileOpen === "animatronics"}
            onToggle={() => toggleMobileSection("animatronics")}
            accent="#81C950"
          >
            {animatronicsProjects.map((project) => (
              <MobileProjectCard key={project.slug} project={project} />
            ))}
          </MobileAccordionSection>
        ...
        <footer className="px-6 py-4 bg-[#FAF0DD]/90 border-t-2 border-[#702C95] text-center">
          <a
            href="mailto:play.lmjambi@gmail.com"
            className="font-dokdo text-[#702C95] text-lg hover:text-[#EC6BA7] transition-colors"
          >
