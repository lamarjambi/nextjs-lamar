...
export function generateStaticParams() {
  return animatronicsProjects.map((p) => ({ slug: p.slug }));
}

export default async function AnimatronicsDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const project = animatronicsProjects.find((p) => p.slug === slug);
  if (!project) notFound();
    ...
    >
      ...
      <main className="px-12 py-12 max-w-5xl mx-auto">
        ...
        <div className="bg-[#FAF0DD]/90 border-2 border-[#702C95] rounded-2xl p-8 space-y-8">
          ...
          {/* tags */}
          <div>
            <p className="font-press-start text-[#702C95] text-xs mb-3">Materials & Techniques</p>
